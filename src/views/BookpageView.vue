...
const fetchPageResults = async () => {
    ...
    try {
        ...
        const query = `SELECT ${index} AS dict, word, meaning FROM ${id} LIMIT ${resultsPerPage} OFFSET ${(pageNum.value - 1) * resultsPerPage}`
        console.log(query)

        const rows = await queryDb(query)
        pageResults.value = rows ? parseDictRows(rows) : [] // null sent when zero results
    } catch(e) {
        console.log(e)
        pageResults.value = []
...
const pageStatus = computed(() => {
  if (isLoading.value) {
    ...
    return { text: `පොතේ ${pageNum.value} පිටුවේ “${pageResults.value[0].word}” වචනයේ සිට “${pageResults.value.slice(-1)[0].word}” වචනය දක්වා වචන ${pageResults.value.length} ක තේරුම් පහතින් බලන්න.`, type: 'success' }
  } else {
  ...
  <div :style="useSettingsStore().fontSizeStyle">
    ...
    <div v-else class="container"> 
      <div class="sm:columns-2 xl:columns-3">
          <DictionaryEntry v-for="(entry, i) in pageResults" :entry="entry" :key="i" class="break-inside-avoid-column"/>
      </div>
    </div>
